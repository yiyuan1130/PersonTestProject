...
public class SDFRendererEditor : Editor {
	...
	void GUI_Sprite(){
		...
		if (GUI.changed){
			if (target._sprite != null){
				target.meshRenderer.sharedMaterial.SetTexture("_MainTex", target._sprite.texture);
			}else{
				target.meshRenderer.sharedMaterial.SetTexture("_MainTex", null);
			}
		}
	}
	...
	void GUI_Style(){
		...
		if (GUI.changed){
			target.meshRenderer.sharedMaterial.SetInt("_Style", (int)target._style);
		}
	}
	...
	void GUI_Ramp(){
		...
		if (GUI.changed){
			if (target._ramp != null){
				target.meshRenderer.sharedMaterial.SetTexture("_RampTex", target._ramp);
			}
			else
			{
				target.meshRenderer.sharedMaterial.SetTexture("_RampTex", null);
			}
		}
	}
	...
	void GUI_Color(){
		...
		if (GUI.changed){
			target.meshRenderer.sharedMaterial.SetVector("_Color", target._color);
		}
	}
	...
	void GUI_Step(){
		...
		if (GUI.changed){
			target.meshRenderer.sharedMaterial.SetInt("_Step", (int)target._step);
		}
	}
